...
function bindStaticListeners() {
  ...
  btnClearWallet.addEventListener("click", () => {
    if (confirm("هل أنت متأكد من رغبتك في إعادة ضبط محفظة المفردات؟ سيتم حذف جميع الكلمات المحفوظة.")) {
      savedWords = [];
      saveStateToStorage();
      updateWalletCountBadge();
...
function stopDetectiveFlashlight(focusStoryBox) {
  ...
  if (focusStoryBox) {
    ...
    const badgeHtml = isDetectiveMode ? `<div class="detective-flashlight-badge">🔦 كشاف المحقق نَشِط — حرك الماوس لإضاءة الكلمات والأدلة بالنص!</div>` : '';
